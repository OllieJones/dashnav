...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const locales = get_locales(dashnav.locale)
        ...
        if (do_sub) {
          ...
          for (const submenu of submenus) {
            ...
            if (link_element) {
              ...
              if (name && link) {
                sub_items.push({label: sub_title.join(dashnav.submenu_delimiter), link, nest_level: 2})
              }
            }
          }
        }
            ...
            if (submenu.classList.contains('wp-submenu-head')) {
              ...
              if (link_element) {
                ...
                if (name && link) {
                  ...
                  title.push(name)
                  sub_items.push({label: title.join(dashnav.submenu_delimiter), link, nest_level: 2})
                }
              }
            }
          }
    ...
    function define_mixins() {
      ...
      String.prototype.normalize_for_search = function () {
        return this
          .trim()
          .replace(dashnav.submenu_delimiter, ' ')
          .normalize('NFD')
